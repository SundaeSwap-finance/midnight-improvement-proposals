...
An on-chain registry SundaeSwap Capacity Exchange Service (CES) servers. Operators register a domain name under a secret key and post collateral; clients discover available CES servers by reading the public ledger state. No off-chain centralized list is needed.


To register, an operator calls `registerServer` with their domain name and an expiry timestamp, and locks a fixed amount of NIGHT as collateral. Each entry is keyed by the `persistentHash` of an operator-held secret and the secret never goes on-chain. The contract enforces domain name uniqueness, making two similar entries impossible. An expiry can't be set further out than `maximumRegistrationPeriod`, which is fixed at contract deployment.

Operators can extend their registration via `renewRegistration`. Overwrites the existing entry with a new expiry while preserving the domain name.
